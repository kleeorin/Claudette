...
for (const u of ENCODINGS) {
  ...
  check(gated, `gated: ${u}`, gated ? '401' : `LEAKED ${r.body.slice(0, 40)}`)
}
...
check(post.statusCode === 401, 'gated: encoded POST /api/session/setSandbox',
  post.statusCode === 401 ? '401' : `ACCEPTED (${post.statusCode}) — confinement disablable unauthenticated`)
...
for (const u of ['/jupyter/api/kernels', '/%6aupyter/api/kernels', '/j%75pyter/api/kernels']) {
  const r = await hit(u)
  check(r.statusCode === 401, `gated: ${u}`, r.statusCode === 401 ? '401' : `LEAKED (${r.statusCode})`)
}

// ── …while everything deliberately public STAYS public ────────────────────────────────
for (const [u, want] of [['/api/health', 200], ['/assets/app.js', 200]] as const) {
  const r = await hit(u)
  check(r.statusCode === want, `still open: ${u}`, `${r.statusCode}`)
}
const authed = await hit('/%61pi/session/list', { headers: { authorization: 'Bearer SECRET-TOKEN' } })
check(authed.statusCode === 200, 'an AUTHENTICATED encoded request still works', `${authed.statusCode}`)
...
check(malformed.statusCode !== 500, 'malformed escape does not 500', `${malformed.statusCode}`)
...
check(csrf.statusCode === 403, '(b) fsApi Sec-Fetch guard survives encoding',
  csrf.statusCode === 403 ? '403' : `cross-site request allowed through (${csrf.statusCode})`)
