...
function roleScopedDenies(def: ConnectorDef, readOnlyRole: boolean): string[] {
  ...
  // Never probed — OR probed to an empty list. Both mean "we do not know this server's
  // tool surface", and an empty list is the easy one to get wrong: a server that answers
  // tools/list with [] (or that we failed to parse) would otherwise read as "probed,
  // nothing to deny" and hand a read-only role the whole server. Fail closed on both.
  if (readOnlyRole && !tools?.length) return [denyAllRule(def.id)]
